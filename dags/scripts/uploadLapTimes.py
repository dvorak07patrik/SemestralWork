...
def transform_and_load_to_db():
	...
	file_path = os.path.join(os.path.dirname(__file__), 'data/lap_times.csv')
	...
	for chunk in pd.read_csv(file_path, chunksize=chunk_size):
		...
		for index, row in chunk.iterrows():
			...
			if race_exists(row['raceId']) and driver_exists(row['driverId']):
		...
		if not processed_df.empty:
			...
			processed_df.to_sql('fact_lap_times', engine, if_exists='append', index=False)
